...
getCookie('deck') === '' ? setDeck('mizu') : setDeck(getCookie('deck'));
...
document.addEventListener('keydown', e => {
  ...
  } else if (!document.querySelector('#deck-center').classList.contains('hidden')) {
    if (e.key === 'Escape'){
      hideDeckCenter();
      inputField.focus();
    }
...
function setDeck(_deck) {
  const deck = _deck.toLowerCase();
  fetch(`decks/${deck}.css`)
    .then(response => {
      if (response.status === 200) {
        response
          .text()
          .then(css => {
            setCookie('deck', deck, 90);
            document.querySelector('#deck').setAttribute('href', `decks/${deck}.css`);
            setText();
          })
          .catch(err => console.error(err));
      } else {
        console.log(`deck ${deck} is undefine`);
      }
    })
    .catch(err => console.error(err));
}
        ...
        response
          ...
          .then(body => {
            ...
            for(i = 0;i < keys.length; i ++){
              ...
              deck.setAttribute('class', 'deck-button');
              deck.setAttribute('onClick', `setDeck('${keys[i]}')`);
              deck.setAttribute('id', keys[i]);

              
              deck.setAttribute('tabindex', i + 5);
              ...
              if(decks[keys[i]]['customHTML'] != undefined){
                deck.style.background = decks[keys[i]]['background'];
                deck.innerHTML = decks[keys[i]]['customHTML']
              }else{
                deck.textContent = keys[i];
                deck.style.background = decks[keys[i]]['background'];
                deck.style.color = decks[keys[i]]['color'];
              }
              document.getElementById('deck-area').appendChild(deck);
            }
          })
          .catch(err => console.error(err));
      } else {
        console.log(`Cant find deck-list.json`);
      }
    })
    .catch(err => console.error(err));
}

document.getElementById('show-decks').addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    showDeckCenter();
    inputField.focus();
  }
...
document.getElementById('show-scores').addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    showScoreCenter();
    inputField.focus();
  }
...
function showDeckCenter() {
  document.getElementById('deck-center').classList.remove('hidden');
  document.getElementById('command-center').classList.add('hidden');
}

function hideDeckCenter() {
  document.getElementById('deck-center').classList.add('hidden');
  document.getElementById('command-center').classList.remove('hidden');
}

function showScoreCenter() {
  document.getElementById('score-center').classList.remove('hidden');
  document.getElementById('command-center').classList.add('hidden');
}

function hideScoreCenter() {
  document.getElementById('score-center').classList.add('hidden');
  document.getElementById('command-center').classList.remove('hidden');
}
